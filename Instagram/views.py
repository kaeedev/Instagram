...
class HomeView(TemplateView):
    template_name = "core/layout/general/home.html"
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            #Ahora nos traemos los posts
            last_posts= Post.objects.filter(user__profile__user__in=seguidos)

        else:
            last_posts = Post.objects.all().order_by('-created_at')[:10]
        context['last_posts'] = last_posts   #Inyectamos en un contexto a la plantilla las 5 ultimas publicaciones
        return context


class LoginView(FormView):
    template_name = "core/layout/general/login.html"
...
class RegisterView(CreateView):
    template_name = "core/layout/general/register.html"
    model = User
...
class LegalView(TemplateView):
    template_name = "core/layout/general/legal.html"


class ContactView(TemplateView):
    template_name = "core/layout/general/contact.html"

@method_decorator(login_required, name='dispatch')
class ProfileDetailView(DetailView, FormView):
    model = UserProfile
    template_name = "core/layout/general/profile_detail.html"
...
class ProfileListView(ListView):
    model = UserProfile
    template_name = "core/layout/general/profile_list.html"
    context_object_name = "profiles"

    def get_queryset(self): #Esto modifica la consulta
        if self.request.user.is_authenticated:
            return UserProfile.objects.all().order_by('user__username').exclude(user=self.request.user) #Para que no enseñe mi propio perfil en la lista de perfiles si estoy logeado
        return UserProfile.objects.all().order_by('user__username') #Para que enseñe mi propio perfil en la lista de perfiles si no estoy logeado

@method_decorator(login_required, name='dispatch') #Protegemos la vista de usuarios no logeados
class ProfileUpdateView(UpdateView):
    model = UserProfile
    template_name = "core/layout/general/profile_update.html"
    context_object_name = "profile"
    ...
    def dispatch(self, request, *args, **kwargs): #Sobreescribimos el dispatch para que un usuario q no sea el del perfil no pueda editar perfiles de otro mediante la URL y lo manda a la home
        user_profile = self.get_object()
        if user_profile.user != self.request.user:
            return HttpResponseRedirect(reverse('home'))
        return super().dispatch(request, *args, **kwargs)
    #Esto lo que hace es antes de cargar la vista de editar perfil, comprueba q estamos editando nuestro perfil y no el de otra persona

@login_required #Protegemos la vista de usuarios no logeados
